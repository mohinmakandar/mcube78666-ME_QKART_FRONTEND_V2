    ...
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          ...
          handleNewAddress({...newAddress,value:""});
          }}
        >
          Add
        </Button>
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const responseData = await axios.post(`${config.endpoint}/user/addresses`,data, {headers:{Authorization: `Bearer ${token}`}});
      
      setAddresses({...addresses,all:responseData.data});
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      const responseData = await axios.delete(`${config.endpoint}/user/addresses/${addressId}`,{ headers: { Authorization: `Bearer ${token}` }});
  ...
  const validateRequest = (items, addresses) => {
    ...
    else if(addresses.all.length<1){
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      let data = {"addressId":""+addresses.selected};
      const responseData= await axios.post(`${config.endpoint}/cart/checkout`,data,{headers:{Authorization: `Bearer ${token}`}});
      ...
      if (e.response) {
        ...
        enqueueSnackbar(
          "Could not perfomr request. Check that the backend is running, reachable and returns valid JSON.",
          {
            variant: "error",
          }
        );
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      if (productsData && cartData) {
        ...
        //setting addresses to state object
        // const addrArray = await getAddresses(token);

        // // console.log("addarray \n",addrArray);
        // // console.log("addresses \n",addresses.all);
        // setAddresses({...addresses,all:addrArray});
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {  
                ...
                addresses.all.map((a)=>(
                   <Box key={a._id} className={addresses.selected===a._id?"address-item selected":"address-item not-selected"}  onClick={()=>{setAddresses({...addresses,selected:a._id});}}>
                    <Typography>{a.address}</Typography>
                    <Button startIcon={<Delete/>} onClick={async ()=>{await deleteAddress(token,a._id)}}>Delete</Button>
                  </Box>  
                ))
                :
                <Typography my="1rem">No addresses found for this account. Please add one to proceed</Typography>
              }
              
            </Box>

            {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display either "Add new address" button or the <AddNewAddressView> component to edit the currently selected address */}
